        ...
        <div class="card">
            <div class="card-body py-0 px-1">
                <ul class="nav nav-tabs" id="brand-tabs" role="tablist">
                    <li class="nav-item {{ (request()->is('admin/subscribers/list/all')) ? 'border-bottom' : '' }}">
                        <a class="nav-link py-3 {{ (request()->is('admin/subscribers/list/all')) ? 'active' : '' }}" id="allreq-tab" href="{{ route('subscribers.list', ['status' => 'all']) }}">All {{ $requests->count() }}</a>
                    </li>
                    <li class="nav-item {{ (request()->is('admin/subscribers/list/2')) ? 'border-bottom' : '' }}">
                        <a class="nav-link py-3 {{ (request()->is('admin/subscribers/list/2')) ? 'active' : '' }}" id="queue-tab" href="{{ route('subscribers.list', ['status' => 2]) }}">Queue {{ $queue->count() }}</a>
                    </li>
                    <li class="nav-item {{ (request()->is('admin/subscribers/list/3')) ? 'border-bottom' : '' }}">
                        <a class="nav-link py-3 {{ (request()->is('admin/subscribers/list/3')) ? 'active' : '' }}" id="progress-tab" href="{{ route('subscribers.list', ['status' => 3]) }}">Progress {{ $progress->count() }}</a>
                    </li>
                    <li class="nav-item {{ (request()->is('admin/subscribers/list/4')) ? 'border-bottom' : '' }}">
                        <a class="nav-link py-3 {{ (request()->is('admin/subscribers/list/4')) ? 'active' : '' }}" id="review-tab" href="{{ route('subscribers.list', ['status' => 4]) }}">Review {{ $review->count() }}</a>
                    </li>
                    <li class="nav-item {{ (request()->is('admin/subscribers/list/0')) ? 'border-bottom' : '' }}">
                        <a class="nav-link py-3 {{ (request()->is('admin/subscribers/list/0')) ? 'active' : '' }}" id="completed-tab" href="{{ route('subscribers.list', ['status' => 0]) }}">Completed {{ $completed->count() }}</a>
                    </li>
                </ul>
            </div>
                    ...
                    <table class="table table-bordered bg-white border-0 table-hover mb-0" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr>
                                    <td class="border-left-0 border-right-0 font-weight-bold"><a class="text-dark" href="{{ route('adminrequest.'. $status, ['customer_id' => $user->uid]) }}">{{ $user->first_name }} {{ $user->last_name }}</a></td>
                                    ...
                                    <td class="d-flex justify-content-end border-left-0 border-right-0">
                                        <a href="{{ route('adminrequest.'. $status, ['customer_id' => $user->uid]) }}"
