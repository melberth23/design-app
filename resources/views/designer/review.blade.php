...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800 page-heading"><a href="{{route('designer.customers', ['status' => 4])}}" class="d-none d-sm-inline-block btn btn-sm btn-outline-light text-dark border"><i class="fas fa-arrow-left fa-sm"></i></a> <em>{{ $customer->first_name }}</em> - For review requests</h1>
        </div>
        ...
        <div class="card">
            <div class="card-body py-0 px-1">
                <ul class="nav nav-tabs" id="brand-tabs" role="tablist">
                    ...
                    <li class="nav-item {{ (str_contains(url()->current(), 'designers/review')) ? 'border-bottom' : '' }}">
                        <a class="nav-link py-3 {{ (str_contains(url()->current(), 'designers/review')) ? 'active' : '' }}" id="review-tab" href="{{ route('designer.review', ['customer_id' => $customer->id]) }}">Review {{ $requests->count() }}</a>
                    </li>
                    <li class="nav-item">
