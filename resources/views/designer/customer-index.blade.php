...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800 page-heading">{{ $status }}</h1>
        </div>
                    ...
                    <table class="table table-bordered bg-white border-0 table-hover mb-0" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr>
                                    <td class="border-left-0 border-right-0 font-weight-bold"><a class="text-dark" href="{{ route('designer.index', ['customer_id' => $user->uid]) }}">{{ $user->first_name }} {{ $user->last_name }}</a></td>
                                    ...
                                    <td class="d-flex justify-content-end border-left-0 border-right-0">
                                        <a href="{{ route('designer.index', ['customer_id' => $user->uid]) }}"
